        ...
        <div className="pdp__grid">
          ...
          <div className="pdp__info">
            ...
            {product.available ? (
              ...
              <p className="pdp__price pdp__price--soon">Binnenkort beschikbaar</p>
            )}
            ...
            {product.available ? (
              ...
              <Link href="/#contact" className="btn btn--tan btn--lg">
        ...
        {others.length > 0 && (
          <section className="pdp__more">
            ...
            <div className="pdp__more-grid">
              {others.map((p) => (
                <Link key={p.id} href={p.available ? p.href : '/#shop'} className="pdp__more-card">
                  <div className="pdp__more-media">
                    <Image src={p.image} alt={p.alt} fill sizes="200px" style={{ objectFit: 'cover' }} />
                    {!p.available && (
                      <div className="pdp__more-soon">
                        <span>BINNENKORT</span>
                      </div>
                    )}
                  </div>
                  <strong>{p.title}</strong>
                  <span>{p.available ? formatPrice(p.price) : 'Binnenkort'}</span>
                </Link>
              ))}
            </div>
          </section>
